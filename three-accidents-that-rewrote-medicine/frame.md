...
- **Collage illustration:** final — hand-authored ink-only inline SVG (stroke/fill `{colors.ink}` on `{colors.bg-canvas}`, no third color) across all 9 scenes, not a placeholder pending generated assets. Chosen deliberately over AI generation: both HeyGen (`/media-use`) and Higgsfield (`/higgsfield-generate`) need an interactive browser OAuth login unavailable in this sandbox, and separately the user is low on Higgsfield credits. Extend this same hand-authored approach for any future scene art — don't reintroduce a generation dependency without checking auth/credits first.
- **Scene authoring model:** scenes are plain `.scene` divs driven entirely by the GSAP timeline (per `/hyperframes-animation` → `transitions/catalog.md`), not `class="clip"` elements with `data-start`/`data-duration`. All 9 scenes follow this consistently — don't mix the two models.
- **File size:** `index.html` is ~600 lines (9 scenes + one shared timeline) and trips the `composition_file_too_large` lint warning. Splitting into `compositions/*.html` sub-comps is the recommended fix, but each scene's internal choreography would need its own local timeline (a sub-comp timeline can't be reached by the main timeline's selectors) — a real refactor, tracked in `BRIEF.md` as a follow-up rather than done opportunistically.
