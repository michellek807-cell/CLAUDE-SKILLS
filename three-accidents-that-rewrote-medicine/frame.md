...
- **Collage illustration:** not yet sourced. `#scene1`/`#scene2` currently hold placeholder ink-only SVG line art (stroke `{colors.ink}`, fill `{colors.bg-canvas}`, no third color) standing in for the willow-bark and paprika-pod pieces described in the blueprint — swap for real illustration via `/media-use` without changing the two-color rule.
- **Scene authoring model:** scenes are plain `.scene` divs driven entirely by the GSAP timeline (per `/hyperframes-animation` → `transitions/catalog.md`), not `class="clip"` elements with `data-start`/`data-duration`. Keep this consistent across all future scenes in this project — don't mix the two models.
