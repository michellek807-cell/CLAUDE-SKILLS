...
- **Collage illustration:** not yet sourced. All 9 scenes currently hold placeholder ink-only SVG line art (stroke `{colors.ink}`, fill `{colors.bg-canvas}`, no third color) — swap for real illustration via `/media-use` without changing the two-color rule.
